...
const AutoCreateView: React.FC = () => {
    ...
    const runAutoFlow = async () => {
        ...
        try {
            ...
            const imageUrl = await generateThumbnailImage(thumbPromptResponse.text ?? 'A cinematic movie poster');
            ...
            console.error(err);
            const errorMessage = err.message || 'Quy trình tự động đã gặp lỗi. Vui lòng thử lại.';
                     ...
                     <div className="space-y-6">
                        {resultStory && (
                            <div>
                                ...
                                <div className="bg-secondary p-4 rounded-lg max-h-48 overflow-y-auto text-dark-text whitespace-pre-wrap">{resultStory.content}</div>
                            </div>
                        )}
                        {resultThumbnail && (
                             <div>
                        ...
                        {resultPrompts.length > 0 && (
                             <div>
                                ...
                                <div className="bg-secondary p-4 rounded-lg max-h-48 overflow-y-auto space-y-2">
                                    {resultPrompts.map(p => <p key={p.id} className="text-dark-text bg-primary p-2 rounded">{p.prompt}</p>)}
                                </div>
                            </div>
                        )}
                    </div>
