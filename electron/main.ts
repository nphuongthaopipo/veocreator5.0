...
// =================================================================
// 2. LOGIC TỰ ĐỘNG HÓA TRÌNH DUYỆT (LOGIC MỚI - THEO DÕI OPERATION)
// =================================================================
ipcMain.on('browser:start-automation', async (event, { prompts }) => {
    ...
    const completedPrompts = new Set(); // Sửa lỗi: Khai báo biến ở đây
    ...
    try {
        ...
        const processingPrompts = new Map(); // Map: promptId -> { operationName, sceneId }
        ...
        page.on('response', async (response) => {
            if (response.url().includes('video:batchAsyncGenerateVideoText')) {
                try {
                    ...
                    // Lỗi này có thể xảy ra nếu response không phải là JSON, bỏ qua
                }
            }
        });
        ...
        while (completedPrompts.size < totalPrompts) {
            ...
            // Theo dõi kết quả
            const operationsToCheck = Array.from(processingPrompts.entries())
                .filter(([id, data]) => data.operationName && !completedPrompts.has(id))
                .map(([id, data]) => ({ operation: { name: data.operationName }, sceneId: data.sceneId, promptId: id }));
